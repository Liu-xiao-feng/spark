...
object test {
  def main(args: Array[String]): Unit = {
    ...
    val rdd3 = rdd2.map(list => (list(0).trim,list(1).trim.toDouble))
    ...
    val rdd5 = rdd4.map(score => (score._1,score._2.sum/score._2.size.toDouble))
    ...
    rdd6.foreach(result => printf(s"%s %.2f\n",result._1,result._2))
